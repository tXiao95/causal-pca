...
estimate_sigma2 <- function(X, residuals, h) {
  ...
  # Ensure variance is strictly positive to prevent division by zero
...
compute_efficient_score_and_update <- function(X, Y, beta, b, h) {
  ...
  # E_n[vec(S_eff) %*% t(vec(S_eff))]
  outer_S_sum <- matrix(0, nrow = p*d, ncol = p*d)
  for (i in 1:n) {
    outer_S_sum <- outer_S_sum + (vec_S[, i] %*% t(vec_S[, i]))
  }
  avg_outer_S <- outer_S_sum / n
