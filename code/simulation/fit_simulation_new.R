...
well_specified_C_fitter <- function(target, C_df, ...) {
  SL_nuisance_fitter(target, C_df, SL.lib = c("SL.glm", "SL.glmnet", "SL.gam", 
                                              "SL.earth", "SL.xgboost"), ...)
}
...
main <- function() {
  ...
  # Save Output
  #out_dir <- here("outputs/simulation/jasa-initial-submission/")
  out_dir <- here("outputs/simulation/jasa-initial-submission/baseline/")
